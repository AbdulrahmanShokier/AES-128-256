...
# =====================================================================
# VERIFIED REFERENCE MODEL: RTL ALIGNMENT
# =====================================================================

def extract_rs_internals(received_sequence, nsym):
    print("=== REED-SOLOMON DECODING TRACE ===")
    
    # Standard syndrome calculation
    # Note: If your hardware processes MSB first (x^n-1), 
    # the library handles this if passed as a standard array.
    synd = rs_calc_syndromes(received_sequence, nsym)
    
    print(f"1. Syndromes (Dec):\n   {list(synd)}")
    print(f"1. Syndromes (Hex):\n   {[hex(s) for s in synd]}")
    
    if max(synd) == 0:
        print("   (All syndromes are zero. No errors detected.)\n")
        return

    # 2. Find Error Locator Polynomial
    locator = rs_find_error_locator(synd, nsym)
    print(f"2. Error Locator (Hex):\n   {[hex(c) for c in locator]}\n")

    # 3. Correct Message
    try:
        rmes, recc, errata_pos = rs_correct_msg(received_sequence, nsym)
        print(f"3. Corrected Message:\n   {list(rmes)}")
    except Exception as e:
        print(f"3. Decoding Failed: {e}")
        
    print("===================================\n")

if __name__ == "__main__":
    # --- RTL CONFIGURATION ---
    # Ensure these match your Verilog parameters exactly
    m = 8
    n = 255
    k = 223
    nsym = n - k
    
    # Initialize GF tables with your primitive polynomial
    init_tables(prim=0x11d, c_exp=m)
    
    # The hex string from your testbench
    hex_stream = ""
    
    # Parse strictly left-to-right to match Verilog sequential input
    raw_list = [int(hex_stream[i:i+2], 16) for i in range(0, len(hex_stream), 2)]
    
    # IMPORTANT: The library expects the list such that index 0 is the coefficient 
    # of the highest degree if encoding, or based on the specific syndrome formula.
    # If your output hex [5b73d...] is a syndrome vector, compare it index-by-index 
    # with [hex(s) for s in synd].
    received_sequence = _bytearray(raw_list)
    
    extract_rs_internals(received_sequence, nsym)