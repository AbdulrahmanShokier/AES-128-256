...
initial begin
    ...
    file_IQ = $fopen("psk8_IQ_data.csv", "w");

    if (!file_IQ) begin
        $display("ERROR: Could not open psk8_IQ_data.csv");
        $finish;
    end
    ...
    send_symbol(3'b001);   // 45°  → I=SQRT2_INV,     Q=SQRT2_INV
    send_symbol(3'b110);   // 180° → I=minus_one,      Q=0
    send_symbol(3'b011);   // 90°  → I=0,              Q=one
    send_symbol(3'b111);   // 225° → I=NEG_SQRT2_INV,  Q=NEG_SQRT2_INV
    ...
    $display("     psk8_IQ_data.csv  (sample, I_out, Q_out, valid_in)");
    $display("==========================================");
