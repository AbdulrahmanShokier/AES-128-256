...
(
    ...
    // ── Round Instances ──
    round_0        round0 (.clk(clk), .rst(rst), .IN(IN), .KEY(k0), .valid(valid), .symbol_tick(symbol_tick), .OUT(r0_out));
    rounds_1_to_9  round1 (.clk(clk), .rst(rst), .IN(r0_out), .KEY(k1), .valid(valid), .symbol_tick(symbol_tick), .OUT(r1_out));
    rounds_1_to_9  round2 (.clk(clk), .rst(rst), .IN(r1_out), .KEY(k2), .valid(valid), .symbol_tick(symbol_tick), .OUT(r2_out));
    rounds_1_to_9  round3 (.clk(clk), .rst(rst), .IN(r2_out), .KEY(k3), .valid(valid), .symbol_tick(symbol_tick), .OUT(r3_out));
    rounds_1_to_9  round4 (.clk(clk), .rst(rst), .IN(r3_out), .KEY(k4), .valid(valid), .symbol_tick(symbol_tick), .OUT(r4_out));
    rounds_1_to_9  round5 (.clk(clk), .rst(rst), .IN(r4_out), .KEY(k5), .valid(valid), .symbol_tick(symbol_tick), .OUT(r5_out));
    rounds_1_to_9  round6 (.clk(clk), .rst(rst), .IN(r5_out), .KEY(k6), .valid(valid), .symbol_tick(symbol_tick), .OUT(r6_out));
    rounds_1_to_9  round7 (.clk(clk), .rst(rst), .IN(r6_out), .KEY(k7), .valid(valid), .symbol_tick(symbol_tick), .OUT(r7_out));
    rounds_1_to_9  round8 (.clk(clk), .rst(rst), .IN(r7_out), .KEY(k8), .valid(valid), .symbol_tick(symbol_tick), .OUT(r8_out));
    rounds_1_to_9  round9 (.clk(clk), .rst(rst), .IN(r8_out), .KEY(k9), .valid(valid), .symbol_tick(symbol_tick), .OUT(r9_out));
    round_10       round10 (.clk(clk), .rst(rst), .IN(r9_out), .KEY(k10), .valid(valid), .symbol_tick(symbol_tick), .OUT(r10_out));
