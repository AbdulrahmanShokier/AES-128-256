...
assign word_tick = symbol_tick && load && (byte_cnt == NUM_BYTES - 1);
...
begin
    if (!rst)
        byte_cnt <= {CNT_W{1'b0}};
    else if (symbol_tick && load) begin
...
// shift-in the incoming byte while load is high
always @(posedge clk_sample)
begin
    if (!rst)
        data_reg <= {REG_WIDTH{1'b0}};
    else if (symbol_tick && load)
        data_reg <= {data_reg[REG_WIDTH - IN_WIDTH - 1 : 0], data_in};
end

// latch the completed 128-bit word on the 16th byte
always @(posedge clk_sample)
begin
    if (!rst)
        data_out <= {OUT_WIDTH{1'b0}};
    else if (word_tick)
        data_out <= {data_reg[REG_WIDTH - IN_WIDTH - 1 : 0], data_in};
end
